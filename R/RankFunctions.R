...
#'
#' @description convertRank converts a rank from its ranking representation to its ordering representation, and vice-versa. The function does not work with partial ranking.
...
#'
#' Let us consider the following example to illustrate both notations: a judge, which has to rank three holidays destinations according to its preferences, 
#' O1 = Countryside, O2 =Mountain and O3 = Sea, ranks first Sea, second Countryside, and last Mountain. The ordering result of the judge is o = (3, 1, 2) 
#' whereas the ranking result is r = (2, 3, 1).
#'
#' 
#' @examples
#' x <- c(2, 3, 1, 4, 5)
...
#' 
#' @description This function takes in input a matrix containing all the observed ranks (a rank can be repeated) and returns a matrix containing all 
#' the different observed ranks with their observation frequencies (in the last column).
#'
#' @param X a matrix containing ranks.
...
#' @title Convert data
#' 
#' @description This function takes in input a matrix in which the m first columns are the different observed ranks and the last column contains the observation frequency, 
...
#'
#' @description This function computes the probability of x according to a multivariate ISR o parameter mu and pi.
#'
